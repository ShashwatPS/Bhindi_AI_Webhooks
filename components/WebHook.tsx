...
const WebhookCard: FC<WebhookCardProps> = ({ trigger, onDelete }) => {
  ...
  const webhookUrl = `${typeof window !== 'undefined' ? window.location.origin : ''}/api/webhook-lifecycle?triggerId=${id}`;
  ...
  const handleCopyUrl = async () => {
    try {
      ...
      setIsCopied(true);
      toast.message("URL copied to clipboard!"); // ← Fixed: Moved toast here
          ...
          <div className="bg-gradient-to-br from-[#1a1a1c] to-[#16161a] rounded-3xl border border-[#2a2a2e]/60 max-w-3xl w-full max-h-[90vh] flex flex-col shadow-2xl shadow-black/40 overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto overflow-x-hidden p-8 space-y-8">
              ...
              <div>
                ...
                <div className="bg-[#1e1e20]/80 rounded-xl p-3 border border-[#2a2a2e]/40 overflow-hidden">
                  <div>
                    <div className="flex items-center justify-between mb-2">
                      <p className="text-xs text-[#9ca3af] uppercase tracking-wide">Webhook URL</p>
                      <button
                        onClick={handleCopyUrl}
                        className="flex items-center justify-center gap-2 px-3 py-1.5 bg-[#2a2a2e]/80 border border-[#3a3a3e]/60 text-[#9ca3af] hover:text-[#00d68f] hover:bg-[#00d68f]/10 hover:border-[#00d68f]/30 transition-all duration-200 rounded-lg text-xs font-medium"
                    ...
                    <div className="relative group">
                      <p className="text-[#f8fafc] font-mono text-sm bg-[#0a0a0a]/50 p-3 rounded-lg border border-[#2a2a2e]/40 overflow-hidden break-all pr-12">
                        {webhookUrl}
                      </p>
                    </div>
                  </div>
